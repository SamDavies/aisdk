...
pub struct LanguageModelOptions {
    ...
    /// OpenAI native web-search controls. When `Some(...)` (even with all
    /// inner fields `None`) the model's built-in web-search capability is
    /// enabled on providers that support it (e.g. the GPT-5 family via the
    /// Vercel AI Gateway). Silently ignored by providers that don't.
...
/// Controls for OpenAI's native web-search capability on supported models.
///
/// All fields are optional; an instance with every field `None` (i.e.
/// `WebSearchOptions::default()`) serialises to `{}` which enables search
/// with provider defaults. Pass via
/// [`LanguageModelRequestBuilder::with_web_search`] or set the
/// [`LanguageModelOptions::web_search_options`] field directly.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct WebSearchOptions {
    /// `"low"` | `"medium"` (default) | `"high"`. Controls how much
    /// retrieved web context is forwarded to the model.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub search_context_size: Option<String>,

    /// Approximate user location to bias geographically-scoped results.
    /// Shape is provider-defined; passed through verbatim.
    #[serde(default, skip_serializing_if = "Option::is_none")]
